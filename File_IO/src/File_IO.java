...
public class File_IO {
    public static void main(String[] args) {
        ...
        String postalCodes[] = createPostalCodeArray();
        ...
        while (true) {
            ...
            if (keyword.equalsIgnoreCase("Add")) {
                ...
                storePostalCodeArray(postalCodes);
                break;
            }
            else {
    ...
    public static String[] sortPostCodes (String[] postalCodes) {
        ...
        for (int i = 0;i < postalCodes.length; i++) {
            ...
            for (int j = 0;j < postalCodes.length-i-1; j++) {
                if (postalCodes[j+ 1] != null && postalCodes[j] != null && postalCodes[j + 1].compareTo(postalCodes[j]) < 0) {
                    swapped = true;
    ...
    public static String[] createPostalCodeArray() {
        int increment = 0;
        File postalCodeLogs = new File("postalCodes.txt");

        String[] postalCodes = new String[10];

        try {
            Scanner input = new Scanner(postalCodeLogs);

            while (input.hasNext()) {
                    postalCodes[increment] = input.nextLine();
                    increment++;

            }
        } catch (FileNotFoundException e) {
            System.out.println("The postal code file was not found.");
            postalCodes = new String[10];
            return postalCodes;
        }
    ...
    public static void storePostalCodeArray(String[] postalCodes) {
        File postalCodeLogs = new File("postalCodes.txt");

        try {
            PrintWriter output = new PrintWriter(postalCodeLogs);

            for (int i = 0;i < postalCodes.length;i++) {
                output.println(postalCodes[i]);
            }
            output.close();
